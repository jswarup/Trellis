...
- `TestCase` is an intrusive linked-list node containing a suite name, test name, function pointer, and next pointer.
- `TestRunner` is the singleton registry and executor. It appends test cases during registration and runs all or filtered tests.
- `TestRegistrar` is an RAII registration object created by the test macro during static initialization.
- `JEEVES_TEST` declares a test function, its static `TestCase`, and its static registrar.
...
2. Static initialization constructs a `TestCase` and `TestRegistrar`.
3. The registrar inserts the case into `TestRunner`'s intrusive list.
