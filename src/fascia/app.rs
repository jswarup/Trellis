...
use crate::fascia::{
    ...
    view_toolbar, ActivityTab, ExplorerAction, ExplorerState, FasciaStyle, FasciaTheme, MenuAction,
...
impl AppState
{
        ...
        match message
        {
            AppMessage::Menu( action) => match action
            {
                ...
                MenuAction::Documentation => {
                    self.status_info.message = "Documentation: https://github.com/iced-rs/iced".to_string();
                }
            },
    ...
    pub fn  view( &self) -> Element< '_, AppMessage>
    {
        ...
        let  active_tab_content: Element< '_, AppMessage> = if let  Some( active_tab) =
            self.tab_manager.active_tab()
        {
            match active_tab.kind
            {
                TabKind::Welcome => view_welcome( palette),
                TabKind::Settings => view_settings(
                    self.theme,
                    palette,
                    self.show_sidebar,
...
fn  view_welcome< 'a>( palette: ThemePalette) -> Element<'a, AppMessage>
{
    ...
    let  action_card = |icon: &'static str, label: &'static str, desc: &'static str, msg: AppMessage| {
        button(
            column![
                row![
                    text( icon).size( 20),
    ...
    let  actions = row![
        action_card( "📄", "New File", "Create an untitled draft buffer", AppMessage::NewFile),
        action_card( "📁", "Explore Files", "Browse folders and drives", AppMessage::SelectActivityTab( ActivityTab::Explorer)),
    ]
    .spacing( 12);
    let  actions_row_2 = row![
        action_card( "🎨", "Switch Native Theme", "Cycle Windows Fluent & Linux Adwaita", AppMessage::ToggleTheme),
        action_card( "⚙", "Configure Settings", "Customize layout and preferences", AppMessage::OpenSettings),
    ]
    .spacing( 12);
    let  content = column![
        title,
...
fn  view_search_sidebar< 'a>( palette: ThemePalette) -> Element<'a, AppMessage>
{
    ...
    let  search_input = text_input( "Search in files...", "")
        .padding( [6, 10]);
...
) -> Element< 'a, AppMessage>
{
    ...
    for &theme_choice in FasciaTheme::ALL
    {
        ...
        let  btn = button(
            row![
                ...
                text( theme_choice.display_name()).size( 13).style( move |_| text::Style {
                    color: Some( palette.text_primary),
                }),
            ]
            .align_y( Alignment::Center),
    ...
    let  layout_header = text( "Layout Options")
        .size( 16)
        .style( move |_| text::Style {
            color: Some( palette.text_primary),
        });
    let  toggle_sidebar_btn = button(
        row![
            text( if show_sidebar { "✓ Sidebar Visible" } else { "✕ Sidebar Hidden" }).size( 13),
        ]
        .align_y( Alignment::Center),
    )
    .padding( [8, 14])
    ...
    let  toggle_statusbar_btn = button(
        row![
            text( if show_status_bar { "✓ Status Bar Visible" } else { "✕ Status Bar Hidden" }).size( 13),
        ]
        .align_y( Alignment::Center),
    )
    .padding( [8, 14])
