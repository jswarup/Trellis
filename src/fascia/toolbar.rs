...
) -> Element< 'a, Message>
{
    let  bar = row![
        ...
        separator( palette),
        toolbar_btn( "🗂", "Sidebar", map_action( ToolBarAction::ToggleSidebar), palette),
        Space::new().width( Length::Fill),
        toolbar_btn( "🎨", "Theme", map_action( ToolBarAction::ToggleTheme), palette),
        toolbar_btn( "⚙", "Settings", map_action( ToolBarAction::OpenSettings), palette),
    ]
    .spacing( 4)
    .padding( [3, 8])
    .align_y( Alignment::Center);
