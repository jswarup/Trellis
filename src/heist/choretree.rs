...
impl Chore
{
    ...
    pub fn FromClosure<F>(doc_str: &'static str, f: F) -> Self
        where F: Fn(&mut dyn IWorker) + Send + Sync + 'static
    {
        Self { _DocStr:    doc_str,
               _Closure:   None,
    ...
    {
        self._Placement = crate::heist::placement::ChorePlacement::Require(worker);
        self
    }
    pub fn Prefer(mut self, worker: u32) -> Self
    {
        self._Placement = crate::heist::placement::ChorePlacement::Prefer(worker);
        self
    }
    pub fn Any(mut self) -> Self
    {
        self._Placement = crate::heist::placement::ChorePlacement::Any;
        self
    }
    pub fn WithPlacement(mut self, placement: crate::heist::placement::ChorePlacement) -> Self
    {
    ...
    pub fn Post(self, maestro: &Maestro, tails: &mut Stash<u16>) -> u16
    {
        ...
        let target_worker = self._Placement.TargetWorker().unwrap_or(maestro.Index());
        let job_id = if let Some(state) = maestro.State() {
            let target_idx = if target_worker < state._SzThreads.max(1) {
                target_worker
            } else {
                0
            };
            let j = state.ConstructJob(target_idx, 0, work);
            state.SetJobPlacement(j, self._Placement);
            j
        } else {
            maestro.ConstructJob(0, work)
        };
    ...
    pub fn Par(self, other: impl Into<ChoreNode>) -> ChoreNode { ChoreNode::Leaf(self).Par(other) }
}
...
impl<'a, T> Clone for SpawnQuellNode<'a, T>
{
    fn clone(&self) -> Self
    {
        Self { _Data:       unsafe { self._Data.Alias() },
...
{
    pub fn New(data: crate::silo::arr::MutArr<'a, T>, target: ChoreTarget, itemWeight: u32,
               docStr: &'static str,
...
impl<'a, T: Send + Sync> From<SpawnQuellNode<'a, T>> for ChoreNode
{
    fn from(val: SpawnQuellNode<'a, T>) -> Self
    {
        ...
        ChoreNode::SpawnQuell(ErasedSpawnQuell { _DataPtr:    val._Data.Data() as usize,
                                                 _DataLen:    val._Data.Size(),
                                                 _ElemSize:   std::mem::size_of::<T>() as u32,
                                                 _DocStr:     val._DocStr,
                                                 _ItemWeight: val._ItemWeight,
                                                 _IsCpu:      matches!(val._Target,
                                                                       ChoreTarget::Cpu),
...
macro_rules! SpawnQuell {
    ( $data:expr, $target:expr, $spawnFn:expr, $quellFn:expr) => {
        $crate::heist::choretree::SpawnQuellNode::New($data,
                                                      $target,
                                                      1,
                                                      "SpawnQuell",
                                                      $spawnFn,
...
{
    fn from(chore: Chore) -> Self { ChoreNode::Leaf(chore) }
}
...
impl BitOr<Chore> for Chore
{
    ...
    {
        ChoreNode::Par(Box::new(ChoreNode::Leaf(self)),
...
{
    type Output = ChoreNode;
    fn bitor(self, rhs: ChoreNode) -> ChoreNode { ChoreNode::Par(Box::new(self), Box::new(rhs)) }
}
impl Shr<Chore> for Chore
{
    ...
    {
        ChoreNode::Seq(Box::new(ChoreNode::Leaf(self)),
                       Box::new(ChoreNode::Leaf(rhs)))
    }
}
...
{
    type Output = ChoreNode;
    fn shr(self, rhs: ChoreNode) -> ChoreNode { ChoreNode::Seq(Box::new(self), Box::new(rhs)) }
}
...
pub fn PostChoreNode(node: ChoreNode, maestro: &Maestro, tails: &mut Stash<u16>) -> u16
{
    match node {
        ...
        ChoreNode::SpawnQuell(sq) => {
            ...
            let quell_job = maestro.ConstructJob(0,
            ...
            if !sq._IsCpu || total_weight <= 20 {
                ...
                let spawn_job = maestro.ConstructJob(quell_job,
                                                     WorkPtr::FromClosure(move |w| {
        ...
        ChoreNode::Coro(coro) => {
            ...
            let job = if let Some(state) = maestro.State() {
                ...
                let j = state.ConstructJob(target_idx,
                                           0,
                                           WorkPtr::FromClosure(move |w| {
                                               crate::heist::corochore::coro_job_func(c,
                                                                                      placement,
                                                                                      0, w);
                                           }));
