...
segue_test!(Swarm, CpuBufferLifecycle, |ctx| {
    ...
    segue_assert_eq!(ctx, buf.Size(), 64);
    segue_assert_eq!(ctx, buf.Label(), "test_buf");

    let data: [u8; 4] = [10, 20, 30, 40];
    let write_res = buf.Write(&data);
    segue_assert!(ctx, write_res.is_ok());

    let mut read_back = buf.Read();
    segue_assert!(ctx, (read_back.Cap() as usize) >= 4);
    segue_assert_eq!(ctx, read_back[0], 10);
    segue_assert_eq!(ctx, read_back[1], 20);
    segue_assert_eq!(ctx, read_back[2], 30);
    segue_assert_eq!(ctx, read_back[3], 40);

    // Verify Read() returns an independent copy
    read_back[0] = 99;
    let second_read = buf.Read();
    segue_assert_eq!(ctx, second_read[0], 10);
});

segue_test!(Swarm, CpuDeviceDoubleOp, |ctx| {
...
segue_test!(Swarm, CpuDeviceVectorAddOp, |ctx| {
    ...
    for (i, &val) in result_floats.iter().enumerate() {
        segue_assert_eq!(ctx, val, (i * 11) as f32);
    }
});

segue_test!(Swarm, SwarmEngineCollatzAndParallelDispatch, |ctx| {
    ...
    for (i, &val) in res_u32.iter().enumerate() {
        segue_assert_eq!(ctx, val, Collatz(in_vals[i as u32]));
    }
});

segue_test!(Swarm, SwarmUnsupportedBackend, |ctx| {
    ...
    segue_assert!(ctx, err.is_err());
    let err_val = err.unwrap_err();
    segue_assert_eq!(ctx, err_val._Kind, SwarmErrorKind::UnsupportedBackend);

    let cuda_dev = ComputeDevice::WithBackend(BackendKind::CudaOxide, 0);
    let err_cuda = cuda_dev.Synchronize();
    segue_assert!(ctx, err_cuda.is_err());
    let cuda_val = err_cuda.unwrap_err();
    segue_assert_eq!(ctx, cuda_val._Kind, SwarmErrorKind::UnsupportedBackend);
});

//-------------------------------------------------------------------------------------------------
// Console & Example Tests

segue_console_test!(Swarm, SwarmConsoleReport, |ctx| {
    ...
    segue_assert_eq!(ctx, engine.Backend(), BackendKind::Cpu);
});

segue_example_test!(Swarm, SwarmVectorAddExample, |ctx| {
    let engine = SwarmEngine::Auto();
    ...
    segue_assert_eq!(ctx, out_floats[3], 44.0);
});
