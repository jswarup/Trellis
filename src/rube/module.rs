...
#[inline]
pub fn EvalRaw(op: KernelOp, in1: u64, in2: u64, mask: u64) -> u64 { op.EvalRaw(in1, in2, mask) }
...
pub fn Eval4State(op: KernelOp, in1: u64, x1: bool, i1: bool, in2: u64, x2: bool, i2: bool,
                  mask: u64)
                  -> Eval4Result
{
...
{
    #[inline]
    pub const fn IsNone(&self) -> bool { matches!(self, Self::None) }
    #[inline]
    pub const fn IsCoro(&self) -> bool { matches!(self, Self::Coro(_)) }
    #[inline]
    ...
    pub fn ClassKey(&self) -> (u8, usize)
    {
        match self {
            ...
            Self::Coro(factory) => {
                let rawDyn: *const (dyn Fn() -> crate::rube::coro_kernel::CoroInstance
                     + Send
                     + Sync) = Arc::as_ptr(factory);
                let vtablePtr = unsafe {
                    std::mem::transmute::<*const (dyn Fn() -> crate::rube::coro_kernel::CoroInstance
                                             + Send
                                             + Sync),
                                        (usize, usize)>(rawDyn).1
                };
                (4, vtablePtr)
            }
        }
...
{
    #[inline]
    pub fn New(op: KernelOp, modStart: u32, count: u32, mask: u64, in1: Buff<TriggerId>,
...
impl Module
{
    pub fn New(id: ModuleId, parent: ModuleId, name: impl Into<String>, inPorts: USeg,
               outPorts: USeg, kernel: KernelKind)
               -> Self
    {
        Self { _Id:          id,
               _Parent:      parent,
