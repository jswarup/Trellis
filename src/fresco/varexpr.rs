...
impl VarAttrib {
    ...
    pub fn IsAggregate(&self) -> bool { self._AggrIndex.is_some() }
    pub fn IsIndependent(&self) -> bool { !self.IsAggregate() && self._DepExpr.is_none() }
    pub fn IsDependent(&self) -> bool { !self.IsAggregate() && self._DepExpr.is_some() }
    pub fn HasBits(&self, kind: VarKind) -> bool { self._Flags & (1 << kind as u32) != 0 }
}
...
#[derive(Clone)]
pub struct VarExpr { _VarIndex: u32 }

impl VarExpr {
    pub fn New(varIndex: impl Into<u32>) -> Self { Self { _VarIndex: varIndex.into() } }
    pub fn VarIndex(&self) -> u32 { self._VarIndex }
}
